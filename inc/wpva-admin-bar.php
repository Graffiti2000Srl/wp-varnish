...
class WP_Varnish_Admin_Bar extends G2K_Admin_Bar {
	public function create_admin_bar(WP_Admin_Bar $wp_admin_bar) {
		...
		$wp_admin_bar->add_node(array(
			...
			'href'   => wp_nonce_url(admin_url('admin.php?page=' . $this->_plugin->slug . '&amp;purge_all&amp;noheader=true'), $this->_plugin->slug),
		));
		...
		if ($post_id = static::_getPostID()) {
			$wp_admin_bar->add_node(array(
				...
				'href'   => wp_nonce_url(admin_url('admin.php?page=' . $this->_plugin->slug . '&amp;purge_this&amp;noheader=true&amp;post_id=' . $post_id), $this->_plugin->slug),
			));
		}
	}
