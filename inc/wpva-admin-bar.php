...
class WP_Varnish_Admin_Bar extends G2K_Admin_Bar {
	public function create_admin_bar(WP_Admin_Bar $wp_admin_bar) {
		$wp_admin_bar->add_node(array(
			...
			'href'   => admin_url('options-general.php?page=' . $this->_plugin->prefix . '_settings'),
		));
		...
		if ($post_id = static::_getPostID()) {
			$wp_admin_bar->add_node(array(
				...
				'href'   => wp_nonce_url(admin_url('options-general.php?page=' . $this->_plugin->prefix . '_settings&amp;' . $this->_plugin->prefix . '_purge_this&amp;noheader=true&amp;post_id=' . $post_id), $this->_plugin->slug),
			));
		}
	}
